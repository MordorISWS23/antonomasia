...
class AntonomasiaGenerator(object):
  ...
  def translate_topk(self, a: str, c: str, k: int = 10, magnitude_sort: bool = False) -> Tuple[np.array, np.array]:
    """
    Retrieve the top k candidates by translating vectors.
    ...
    cos_sim = cosine_similarity(a_proj.reshape(1, -1), b_emb).reshape(-1)
